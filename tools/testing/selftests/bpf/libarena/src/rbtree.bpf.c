...
rbtree_t rb_create(enum rbtree_alloc alloc, enum rbtree_insert_mode insert)
{
	rbtree_t rbtree;
...
int rbnode_rotate(rbtree_t rbtree, rbnode_t node, int dir)
{
	rbnode_t tmp, parent;
	int parentdir;
...
static inline rbnode_t rb_node_alloc_common(rbtree_t rbtree, u64 key, u64 value)
{
...
__weak
rbnode_t rb_node_alloc(rbtree_t rbtree, u64 key, u64 value)
{
	if (!rbtree) {
	...
	return rb_node_alloc_common(rbtree, key, value);
}
...
static
int rb_node_insert(rbtree_t rbtree, rbnode_t node)
{
...
__weak int rb_least(rbtree_t rbtree, u64 *key, u64 *value)
{
	rbnode_t least;
	if (!rbtree->root)
		return -ENOENT;
...
static
int rb_node_remove(rbtree_t rbtree, rbnode_t node)
{
...
__weak
enum rb_print_state rb_print_next_state(rbnode_t rbnode, enum rb_print_state state, u64 *next)
{
	if (unlikely(!next))
 ...
 */
__weak
int rb_print_pop_up(u64 *rbnodep, u8 *depthp, enum rb_print_state (*stack)[RB_MAXLVL_PRINT], enum rb_print_state *state)
{
	rbnode_t rbnode;
	volatile u8 depth;
	int j;
...
int rb_print(rbtree_t rbtree)
{
	...
	while (can_loop) {
		...
		rbnode_addr = (u64)rbnode;
		ret = rb_print_pop_up(&rbnode_addr, &depth, &stack, &state);
		rbnode = (rbnode_t)rbnode_addr;
		if (ret)
			return -EINVAL;
