...
static inline int arena_memset(s8 __arena *dst, s8 val, size_t size)
{
	 ...
	 *
	 * The N LSBs of dst and -dst add to 1 << (N+1), which
	 * is why
	 *
	 * dst + (-dst) = 0x0ULL through overflow. So the last N = 3
	 * bits of the negative are the number of bytes to align
	 * dst on the last 3 bits.
	 *
	 */
...
ptraligned:
	 ...
	 * Shifting by by 1 bytes is equal to multiplication by 0x01 << 8,
	 * ...
	 * Shifting by by 14 bytes is equal to multiplicatio by 0x01 << 56.
	 *
