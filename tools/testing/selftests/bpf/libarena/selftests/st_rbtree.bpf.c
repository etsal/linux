...
struct rbtree_test_ctx {
	struct rbnode rbnode;
	struct rbtree_test_ctx __arena *next;
};

typedef struct rbtree_test_ctx __arena rbtree_test_ctx_t;
...
__weak int test_rbtree_find_nonexistent(rbtree_t __arg_arena *rbtree)
{
	u64 key = 0xdeadbeef;
	u64 value = 0;
	int ret;
...
__weak int test_rbtree_insert_existing(rbtree_t __arg_arena *rbtree)
{
	u64 key = 525252;
	u64 value = 24;
	int ret;
...
__weak int test_rbtree_update_existing(rbtree_t __arg_arena *rbtree)
{
	u64 key = 33333;
	u64 value;
	int ret;
...
__weak int test_rbtree_insert_one(rbtree_t __arg_arena *rbtree)
{
	u64 key = 202020;
	u64 value = 0xbadcafe;
	int ret;
...
__weak int test_rbtree_insert_ten(rbtree_t __arg_arena *rbtree)
{
	u64 key, value;
	int ret, i;
...
__weak int test_rbtree_duplicate(rbtree_t __arg_arena *rbtree)
{
	u64 key = 0x121212;
	u64 value;
	int ret, i;
...
__weak int test_rbtree_insert_many(rbtree_t __arg_arena *rbtree)
{
	const size_t numkeys = sizeof(keys) / sizeof(keys[0]);
	rbtree_test_ctx_t *taskc;
	u64 key, value;
	int ret;
	int i;
	...
	for (i = 0; i < numkeys && can_loop; i++) {
		...
		if (rbtree->alloc != RB_ALLOC) {
			taskc = malloc(sizeof(*taskc));
			if (!taskc) {
				arena_stdout("out of memory");
				return -ENOMEM;
			}
...
__weak int test_rbtree_remove_one(rbtree_t __arg_arena *rbtree)
{
 ...
 */
__weak
u64 remove_key(rbtree_t __arg_arena *rbtree, rbtree_test_ctx_t __arg_arena *taskc, u64 key, int *ret)
{
	rbtree_test_ctx_t *tmp;
...
__weak int test_rbtree_remove_many(rbtree_t __arg_arena *rbtree)
{
	const size_t numkeys = sizeof(morekeys) / sizeof(morekeys[0]);
	rbtree_test_ctx_t *taskc = NULL, *first = NULL, *last = NULL;
	u64 key, value;
	int errval = 1;
	int ret;
	int i;
	...
	for (i = 0; i < numkeys && can_loop; i += 2) {
		...
		first = (rbtree_test_ctx_t *)remove_key(rbtree, first, key, &ret);
		if (ret) {
...
__weak int test_rbtree_add_remove_circular(rbtree_t __arg_arena *rbtree)
{
...
__weak int test_rbtree_add_remove_circular_reverse(rbtree_t __arg_arena *rbtree)
{
	...
	bpf_for(i, 0, prefill) {
		...
		if (ret) {
			arena_stdout("Did not remove %d, error %d", prefix - i, ret);
			return errval + i;
		}
	}
...
__weak int test_rbtree_least_pop(rbtree_t __arg_arena *rbtree)
{
...
__weak int test_rbtree_alloc_check(rbtree_t __arg_arena *rbtree)
{
...
__weak int test_rbtree_print(rbtree_t __arg_arena *rbtree)
{
	rb_print(rbtree);
	return 0;
}

#define RBTREE_SELFTEST(suffix, rbtree) SELFTEST(test_rbtree_ ## suffix, (rbtree))
...
int test_rbtree(void)
{
	...
	RBTREE_SELFTEST(find_nonexistent, standard);
	RBTREE_SELFTEST(insert_one, update);
	RBTREE_SELFTEST(print, update);
	RBTREE_SELFTEST(insert_existing, standard);
	RBTREE_SELFTEST(update_existing, update);
	RBTREE_SELFTEST(duplicate, duplicate);
	RBTREE_SELFTEST(insert_ten, update);
	RBTREE_SELFTEST(insert_many, update);
	RBTREE_SELFTEST(insert_many, noalloc);
	RBTREE_SELFTEST(remove_one, standard);
	RBTREE_SELFTEST(remove_many, update);
	RBTREE_SELFTEST(remove_many, noalloc);
	RBTREE_SELFTEST(add_remove_circular_reverse, update);
	RBTREE_SELFTEST(add_remove_circular, update);
	RBTREE_SELFTEST(alloc_check, standard);
